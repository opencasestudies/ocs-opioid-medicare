...
partdinfodescription %>%
  select(`Column Name`,Description)  %>% data.frame(.) %>%
  kable() %>%
  kable_styling() %>%
...
### Can you visualize the top five States with the highest and lowest opioid prescription in Part D in 2016 among the States (ie, excluding places with known area labe such as Armed Forces Pacific)?      
```{r fig.width=8,fig.height=10}
p <- prescriber2016byState %>% 
                    filter(region != "Others") %>%
                    arrange(.,-total.opioid.claim) %>%
