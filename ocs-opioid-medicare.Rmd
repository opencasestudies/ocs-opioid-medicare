...
Q: How many people are there enrolling in this plan?  > 43 million?
[Source 1](https://www.kff.org/medicare/fact-sheet/an-overview-of-the-medicare-part-d-prescription-drug-benefit/), and
[Source 2](https://www.kff.org/medicare/issue-brief/medicare-part-d-in-2018-the-latest-on-enrollment-premiums-and-cost-sharing/). 
...
```{r}
population <- read_csv("./doc/enrollnumber.csv") %>%
...
## Trend of opioid prescription claims by States in 2013 - 2016  
We may want to see the trend of opioid prescription per 100 enrollment during 2013 and 2016 by State. 
```{r fig.width=12,fig.height=10}
library(ggplot2)
...
p + theme_bw() + 
    theme(plot.title = element_text(hjust = 0.5 ), 
          axis.text.x = element_text(angle = 90, hjust = 1,vjust = 0.5)) + 
    ggtitle("Trend of opioid prescription per 100 enrollments in Medicare Part D\ by State during 2013 - 2016") + 
    xlab("States") + 
...
## Trend of total claims by States in 2013 - 2016  
We may want to see the trend of total claims per 100 enrollment during 2013 and 2016 by State. 
```{r fig.width=12,fig.height=10}
library(ggplot2)
...
p + theme_bw() + 
    theme(plot.title = element_text(hjust = 0.5 ), 
          axis.text.x = element_text(angle = 90, hjust = 1,vjust = 0.5)) + 
    ggtitle("Trend of total claims per 100 enrollments in Medicare Part D by State during 2013 - 2016") + 
    xlab("States") + 
...
knitr::include_graphics('./img/map2016.png')
```
