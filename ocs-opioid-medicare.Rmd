...
January 2006. There are two ways for participaints to
...
([For more information on elibility over 65 years old, click here](https://www.hhs.gov/answers/medicare-and-medicaid/who-is-elibible-for-medicare/index.html)
...
undersand the meanings of the variables easily. 
...
deomonstrate how to download file from the website directly using the method as 
...
# Data Import 
**Do you want to describe how to use readr again?**
```{r}
library(readr)
library(tidyverse)
...
populationstate <- states %>% 
                dplyr::left_join(.,population, by = c("state_name"))
```
...
labelled as `Others` in thier `region` variable, we can limit the analysis 
...
### (3) How do you feel the results? Do the results surprise you?               
...
p + scale_y_continuous(labels = comma) + 
    ...
    ggtitle("Number of enrollments vs. Opioid prescription claims in Medicare Part D\ by State in 2015",
            subtitle = "Number of enrollments is associated with number of opioid prescription") + 
...
p + scale_y_continuous(labels = comma) + 
    ...
    ggtitle("Total claims vs. Opioid prescription claims in Medicare Part D\ by State in 2015",
            subtitle = "Number of total claims is associated with number of opioid prescription") + 
...
the rank based on the method I (state opioidd prescrition rate), 
